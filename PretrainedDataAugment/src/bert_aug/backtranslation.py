...
def main():
    ...
    print(args)
    backtranslation_using_en_it_model(args)


def backtranslation_using_en_it_model(args):
    ...
    # load the best model
    en_it_model = MarianModel.from_pretrained(
        os.path.join(args.cache, "wmt19.en-de.joined-dict.single_model")
    )

    it_en_model = MarianModel.from_pretrained(
    ...
    for example in train_examples:
        text = example.text_a
        it_example = en_it_model.translate(text, remove_bpe=True)
        back_translated_example = it_en_model.translate(it_example, remove_bpe=True)
